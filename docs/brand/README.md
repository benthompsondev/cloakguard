...
## Wordmark — white "Cloak" + green "Guard"
Split the display wordmark into two colored spans. `aria-label="CloakGuard"` on the
wrapper, `aria-hidden` on the parts, so screen readers read "CloakGuard" once.
Use it for the **display wordmark only** (Header brand + AboutView `<h1>`), never in body prose.
- "Cloak" = `#F2F4F5` (near-white)
- "Guard" = `#34D399` (accent green)

## Palette
| Token | Hex | Use |
| --- | --- | --- |
| white | `#F2F4F5` | wordmark "Cloak", light text on dark |
| accent | `#34D399` | wordmark "Guard", shield, pill |
...
- Build `ShieldLogo.tsx` as a flat SVG matching the **Full color** variant in
  ...
  secure-indicator pill with three dark dots. Keep `role="img"` + `aria-label="CloakGuard logo"`
