...
| 2026-07-05 | Linux x86_64 target: platform-split Tauri config (shared file + windows/linux overlays), `.deb` + AppImage bundles, an Ubuntu 22.04 CI lane, and a validated multi-platform updater manifest | Same scanner, same privacy model, no new runtime dependencies; the AppImage carries auto-updates while the `.deb` follows a manual package-update path |
| 2026-07-06 | v1.1.0: filter common PowerShell noise from review candidates, add distinctive-prefix provider tokens, and ship Windows plus Linux from one release | Keep human review useful on real scripts while making dual-platform installs and updates honest about AppImage versus `.deb` behavior |
| 2026-07-06 | v1.1.1: add complete Debian descriptions and validated AppStream metadata; make review suggestions dismissible for the current session | Improve Linux package-manager presentation while keeping suggestion choices memory-only and separate from saved preferences |
| 2026-07-06 | v1.1.2: route project links through a two-origin desktop opener scope, recognize AWS temporary access keys, and ship AppStream metadata in both Linux formats | Fix dead webview links without broad URL access, tighten a common cloud-credential check, and make the AppImage easier to list |
